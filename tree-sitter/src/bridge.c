...
void ts_parser_log_to_stderr(TSParser *parser)
{
    ts_parser_set_logger(
        parser, (TSLogger) {.log = log_to_stdout, .payload = NULL});
}
...
bool ts_tree_cursor_current_node_p(const TSTreeCursor *cursor, Node *outNode)
{
    ...
    if (!ts_node_is_null(tsNode)) {
        ts_node_poke(
            ts_tree_cursor_current_field_name(cursor), tsNode, outNode);
    }
    return false;
}

uint32_t ts_tree_cursor_copy_child_nodes(
    TSTreeCursor *cursor, Node *outChildNodes)
{
    assert(cursor != NULL);
    assert(outChildNodes != NULL);
    uint32_t count = 0;
...
TSQuery *ts_query_new_p(
    const TSLanguage *language, const char *source, uint32_t length)
{
...
void ts_query_cursor_exec_p(
    TSQueryCursor *cursor, const TSQuery *query, TSNode *node)
{
    assert(node != NULL);
